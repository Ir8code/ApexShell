...
const INBOUND = {
  ...
  todoGet: (m) => (str(m.id) ? { type: 'todoGet', id: m.id } : null),
  ...
  // persona seats only — no terminal/launch/chrome (or install-specific
  // tags like homelab) from the phone.
  // An unknown persona name falls through presets and opens a plain seat,
  // same as the desktop rail.
  seatCreate: (m) => (str(m.persona)
